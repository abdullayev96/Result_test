...
def message_handler(update, context):
    ...
    state = context.user_data.get("state", 0)
    db_user = db.get_user_by_chat_id(user.id)
    db_user1 = db.get_user_by_id_number(message)
    print("Message: ", db_user1)
    ...
    # if message == "Test natijasini barchaga jo'natish":
    #     users = db.get_all_number()
    #     list1 = []
    #     for u in users:
    #         list1.append(u['id_number'])
    #     for ch in list1:
    #         try:
    #             news = db.get_test_result()
    #             last_news = news[-1]
    #             date = last_news["posted_at"].split(' ')[0]
    #
    #             if last_news['image'] == False:
    #                 context.bot.send_message(
    #                     chat_id=ch,
    #                     text=f"""<b>{last_news[f'heading_{globals.LANGUAGE_CODE[db_user1["lang_id"]]}']}</b>\n{last_news[f'text_{globals.LANGUAGE_CODE[db_user1["lang_id"]]}']}\n<i>{date}</i>""",
    #                     parse_mode="HTML"
    #                 )
    #             else:
    #                 path1 = settings.MEDIA_ROOT
    ...
    #                     chat_id=ch,
    #                     photo=open(f'{newPath}/{last_news["image"]}', "rb"),
    #                     caption=f"""<b>{last_news[f'heading_{globals.LANGUAGE_CODE[db_user1["lang_id"]]}']}</b>\n{last_news[f'text_{globals.LANGUAGE_CODE[db_user1["lang_id"]]}']}\n<i>{date}</i>""",
    #                     parse_mode="HTML"
    #                 )
    #         except Exception:
    #             continue
    ...
    if state == 0:
        ...
        #                  f"📥 <b>Ariza topshirgan kurslarim:</b> \n"
        #                  f"{text}",
        #             parse_mode='HTML'
        #         )
        #
        ...
        elif message == globals.BTN_TEST[db_user["lang_id"]]:
            update.message.reply_text(
                text=globals.TEXT_ENTER_ID[db_user['lang_id']],
                parse_mode="HTML"
            )

            context.user_data["state"] = globals.STATES["user"]
        ...
        test = db.get_user_by_id_number(message)
        print("Test:", test)
        ...
        # update.message.reply_text(
        #     text=f"ID raqam togri kiritilgan bo'lsa \n  tez orada natijangz yuboriladi !!!"
        # )
        # id_number = update.message.text
        # db.create_test(user.id, user.username, id_number)
        # context.bot.send_message(
        #     chat_id=ADMIN_ID,
        #     text=f"<i>{user.username}:</i>\n{id_number}",
        #     parse_mode='HTML'
    ...
    # else:
    #     update.message.reply_text("Assalamu alaykum Iltimos kuting !!!!")
...
def location_handler(update, context):
    ...
    context.bot.send_message(
        ...
        text="Sizni arizangz  qabul qilindi. Adminllarimiz tez orada siz bilan bog'lanadi"
    )

    methods.send_main_menu(context, update.message.from_user.id, db_user['lang_id'])

######################   integratisiya django  +++bot
